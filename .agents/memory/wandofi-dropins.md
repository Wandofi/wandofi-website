...
- Preview must support the same directory URLs as GitHub Pages, including nested blog and service routes. **Why:** Vite's default SPA fallback previously showed the homepage for every menu destination. **How to apply:** preserve directory routing when changing the dev server; verify normal menu URLs rather than working around failures with explicit index.html links.
