...
- In the hero, that single light is also taken literally as one glowing horizontal line: `.hero__line` (a full-viewport-width element inside `.hero__stage`, positioned at `top:50%` so it crosses the wordmark's vertical centre). The line itself is the `::before` (2px red + bloom box-shadow); the `::after` is a soft `mix-blend-mode:screen` bloom band. It sits at `z-index:0` so the wordmark (z1) and portrait (z2) stay in front and the figure occludes it. This is the graduated "Linha — luz única literal" hero exploration.
