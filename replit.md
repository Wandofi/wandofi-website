...
- Home nav now mixes anchors and pages (2026-07-29): Trabalho `#trabalho` · SEO `/seo/` (always visible, incl. mobile) · Tráfego `/gestao-de-trafego/` and Sites `/criacao-de-sites/` (both `.nav__link--extra`, hidden ≤880px) · Contacto `#contacto`. The `[6]` nav counters were removed. `index.html` now also loads `/pages.css` (for `.card__more` links on service cards: SEO técnico → `/seo/seo-tecnico/`, Migração/Construção → `/criacao-de-sites/`, Automações → `/automacoes/`, plus "Ver todos os serviços de SEO" → `/seo/`).
- Nav responsive tiers: desktop shows the `.nav__status` pill + full links capsule; ≤1040px swaps the pill for a `.nav__brand` "W." mark; ≤880px the capsule keeps only Trabalho + Contacto (`.nav__link--extra` links hidden) — mobile always has navigation.
