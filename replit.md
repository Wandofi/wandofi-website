...
- Nav responsive tiers: desktop shows the `.nav__status` pill + full links capsule; ≤1040px swaps the pill for a `.nav__brand` "W." mark; ≤880px the capsule becomes a compact `Menu` disclosure with all five destinations (Trabalho, SEO, Tráfego, Sites, Contacto) in a two-column panel, with Contacto full-width.
